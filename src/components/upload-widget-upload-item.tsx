...
export function UploadWidgetUploadItem({
  ...
  function handleCopyRemoteUrl() {
    if (upload.remoteUrl) navigator.clipboard.writeText(upload.remoteUrl)
  }

  function handleDownloadFile() {
    if (upload.remoteUrl) downloadUrl(upload.remoteUrl)
  }

  return (
    <motion.div
      className="p-2 rounded-lg flex flex-col gap-3 shadow-shape-content bg-white/2 relative overflow-hidden"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
